...
st.markdown(
    ...
    financial value, and relationship strength to identify customers
...
try:
    ...
    st.error(
        """
         **European_Bank.csv not found**

        Please make sure that `European_Bank.csv` and `app.py`
...
if 1 in card_churn.index:

    card_churn_rate = card_churn[1]
...
if 0 in card_churn.index:

    no_card_churn_rate = card_churn[0]

else:

    no_card_churn_rate = 0


credit_card_stickiness = (
    no_card_churn_rate - card_churn_rate
)
...
if churn_filter == "Retained Customers":

    filtered_df = filtered_df[
        filtered_df["Exited"] == 0
    ]

elif churn_filter == "Churned Customers":
...
with tab2:
    ...
    st.header(
        "🛍️ Product Utilization Analysis"
    )
...
st.caption(
    ...
    "Retention Strategy | Streamlit Dashboard"
)